...
function HomePage() {
  ...
  const isCompletedToday = sessions[0]?.scheduled_date === new Date().toISOString().slice(0, 10);
  const totalMinutes = today.phase.exercises.reduce((sum, ex) => {
    ...
    return sum + Math.round(((ex.sets ?? 3) * (ex.reps ?? 12) * 3 + (ex.rest_seconds ?? 30) * ((ex.sets ?? 3) - 1)) / 60);
  }, 0);
  const protocolComplete = prescription.status === "completed";
    ...
    <div className="px-5 pt-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Fase {progress.current_phase} · {today.phase.name}</p>
        </div>
      ...
      <section className="mt-6 rounded-3xl bg-gradient-to-br from-primary-dark to-primary p-5 text-primary-foreground shadow-md">
        ...
        <p className="mt-1 text-sm text-white/80">
          Sessão {today.sessionNumber} de {today.sessionsInPhase} · {today.phase.exercises.length} exercícios · ~{totalMinutes} min
        </p>
        {protocolComplete ? (
          <div className="mt-4 rounded-xl bg-white/15 p-3 text-sm">
            🏆 Protocolo concluído. Parabéns!
          </div>
        ) : isCompletedToday ? (
      ...
      <div className="mt-5 rounded-2xl border-l-4 border-primary bg-bg-subtle p-4">
        <p className="text-sm font-medium text-foreground">
          {getDynamicMessage(progress, progress.pain_history)}
        </p>
      </div>

      <section className="mt-5 grid grid-cols-3 gap-3">
        <StatCard icon={<Flame className="h-4 w-4 text-warning" />} value={`${progress.current_streak}`} label="dias seguidos" />
        <StatCard icon={<TrendingUp className="h-4 w-4 text-success" />} value={`${progress.adherence_rate}%`} label="completo" />
        <StatCard icon={<CheckCircle2 className="h-4 w-4 text-primary" />} value={`${progress.total_sessions_completed}`} label="sessões" />
      </section>
            ...
            sessions.slice(0, 3).map((s) => (
              <div key={s.id} className="flex items-center gap-3 rounded-2xl border border-border bg-card p-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-foreground">Sessão {s.session_number} · Fase {s.phase_number}</p>
